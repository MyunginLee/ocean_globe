...
VAOMesh earth_sphere, back_mesh, extra, target;
VAOMesh pic[12][2];
...
struct MyApp : public App
{
  ...
  Parameter density{"Density", 1, 1, 10};
  Parameter stressor{"Stressors ", 0, 0, 1};
  ...
  void onCreate()
  {
    ...
    for (int d = 0; d < years; d++)
    {
      ostringstream ostr;
      ostr << "../texture/sst/sst_05_" << d + 2003 << "_equi.png"; // ** change stressor
      // ostr << "../texture/nutrient/nutrient_pollution_impact_010_" << 2003 << "_equi.png"; // ** change stressor
      char *filename = new char[ostr.str().length() + 1];
      strcpy(filename, ostr.str().c_str());
    ...
    data_W = oceanData[0][0].width();
    data_H = oceanData[0][0].height();
    ...
    for (int p = 0; p < 2; p++)
    {
      for (int d = 0; d < years; d++)
      {
        for (int row = 0; row < data_H; row++)
        {
          ...
          {
            auto pixel = oceanData[d][p].at(column, data_H - row - 1);
            // if( pixel.r > 80)
            //  && pixel.r < 170)
            // if( pixel.r < 170)
            if (pixel.r)
            {
              ...
              pic[d][p].vertex(x * point_dist + shift, y * point_dist, z * point_dist);
              ...
              pic[d][p].color(RGB(0.2 + pixel.r / 60, 0.2 + pixel.r / 70, 0.5 + pixel.r / 80)); // sst
                                                                                                // pic[d].color(RGB( 0.3+ pixel.r/55, 0.2 + pixel.r/60, 0.2+pixel.r/70)); // sst
  ...
  void onDraw(Graphics &g)
  {
    ...
    if (draw_data)
    {
      ...
      if (pointssize < 1)
        pointssize = 1;
      // g.pointSize(500/nav().pos().magSqr() * density);
      // cout << 200/nav().pos().magSqr() << endl;
      g.translate(Vec3f(shift, 0, 0));
      if (box_draw)
      {
        g.draw(pic[(int)year-2003][(int)stressor]);
        cout << year << endl;
      }
      g.popMatrix();
    }
